...
import com.chirikualii.materiapi.remote.ApiClient
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            val response = ApiClient.service.getMovies("7")

            Log.d("MainActivity::clas.java", "listMovie: ${Gson().toJsonTree(response.body())}")

            val data = response.body()

            val listMovie = data?.results?.map {
                    result ->  Movie(title = result.title, genre =  result.releaseDate, imagePoster = result.posterPath)
            }
